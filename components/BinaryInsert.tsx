  ...
  return (
    <div className="border border-line bg-white p-4">
      <p className="text-sm text-ink/70">
        Inserção binária · {athlete} · {steps === 1 ? "1 comparação" : `${steps} comparações`}
      </p>
      {probe.kind === "probe" ? (
        <div className="mt-3">
          <p>
            A onda nova é melhor ou pior que a de <strong>{board[probe.index]?.name}</strong>, hoje em{" "}
            {board[probe.index]!.place}º?
          </p>
...
export function DirectPlaceForm({ entryId, maxPlace }: { entryId: string; maxPlace: number }) {
  ...
  return (
    <form action={action} className="flex flex-wrap items-end gap-2">
      ...
      {result.saved ? <span className="text-sm text-ocean">Publicado.</span> : null}
    </form>
  );
}
